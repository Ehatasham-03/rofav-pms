...
function ListPropertyBookings() {
  ...
  useEffect(() => {
    ...
    (async function () {
      try {
        const response = await api(`/bookings/${propertyChannexId}`, {}, 'get');
        setBookings(response.data || []);
        let roomTypeRes = await api(
          `/property-room-type/${propertyChannexId}`,
          {},
          'get'
        );
        setRoomTypes(roomTypeRes.data || []);
        const resOngoing = await api(
          `/booking-ongoing/${propertyChannexId}`,
          {},
          'get'
        );
        setBookingOngoing(resOngoing.data || []);
        const resHistory = await api(
          `/booking-history/${propertyChannexId}`,
          {},
          'get'
        );
    ...
    <>
      <div className="block md:grid grid-cols-2 items-center mb-[30px]">
        ...
        <div className="flex justify-end">
          <Link to={`/property/${propertyChannexId}/bookings/create`}>
