...
function ListPropertyBookings() {
  const user = JSON.parse(localStorage.getItem("_session"));
  let formatter = useDateFormatter({ dateStyle: "full" });

  const {setBookbookingCID , bookingCID , properties ,selectedProperty,fetchProperties } = useContext(InventoryContext);
  ...
  const [activeListingTab, setActiveListingTab] = useState(1);
  ...
  const [bookingOngoing, setBookingOngoing] = useState([]);
  ...
  const handleListingTabClick = (index) => {
    setActiveListingTab(index);
  };
  ...
  useEffect(() => {
    ...
    (async function () {
      try {
        if (bookingCID) {
          let propertyData = bookProperties.find(
            (e) => e.value == bookingCID
          );
          ...
          const resOngoing = await api(
            `/booking-ongoing/${bookingCID}`,
            {},
            "get"
          );
          setBookingOngoing(resOngoing.data || []);
          const resHistory = await api(
            `/booking-history/${bookingCID}`,
            {},
            "get"
          );
          setBookingHistory(resHistory.data || []);
          // console.log('propertyData',propertyData);
          
          const responsePolicy = await api(
            `/cancel-policy/${selectedProperty}`,
            {},
            "get"
          );
          setPolicies(responsePolicy.data || []);
        }
      } catch (err) {
        console.log(err);
      ...
      if (values.id) {
        ...
        response = await api(
          `/checkin`,
          {
            ...
            // checkOutDate: Date.parse(bookingDetails?.departure_date),
            notes: rest.is_complimentary_upgrade ? rest.notes : "",
            changedRoomType: rest.changedRoomType || rest.roomType,
          },
          "post"
  ...
  const handleNoShow = async (booking_id = "") => {
    try {
      let response = await api(`/no-show/${booking_id}`, {}, "get");
      if (response) {
        toast.success(response.success);
      }
      setReload(!reload);
  ...
  const handleAcknowledge = async (revision_id) => {
    try {
      const response = await api(
        `/acknowledge-revision/${revision_id}`,
        {},
        "get"
      );
      toast.success(response.success);
  ...
  const handleCancelPolicy = async () => {
    try {
      if (!selectedCancellationPolicy) {
        toast.error("Please select cancellation policy");
        return;
      }
      const response = await api(
        `/cancel-booking/${bookingDetails?.booking_id}`,
        { cancel_policy: selectedCancellationPolicy },
        "post"
      );
      toast.success(response.success);
      onOpenChange();
      ...
      <div className="flex justify-between items-center mb-[30px]">
        ...
        <div className="flex justify-end">
          {/* <Select
            ...
            size="lg"
            variant="flat"
            classNames={{ trigger: "border w-[225px]" }}
            items={properties}
            onChange={(e) => {
              setbookingCID(e.target.value);
            }}
          >
            {(property) => (
              <SelectItem key={property.value}>{property.label}</SelectItem>
            )}
          </Select> */}
        ...
        <div>
          <div className="flex gap-[30px] bg-[#F8F8F8] rounded-lg p-2 mb-4 w-full overflow-scroll text-nowrap">
            <span
              ...
              onClick={() => handleListingTabClick(1)}
            >
              Bookings
            </span>
            <span
              ...
              onClick={() => handleListingTabClick(2)}
            >
              In-House Guests
            </span>
            <span
              ...
              onClick={() => handleListingTabClick(3)}
            >
              Booking History
            </span>
          </div>
          {activeListingTab == "1" ? (
            <Table aria-label="Example empty table">
              <TableHeader>
                <TableColumn key={"title"}>Unique Id</TableColumn>
                <TableColumn key={"customer"}>Guest</TableColumn>
                <TableColumn key={"dates"}>Dates</TableColumn>
                <TableColumn key={"rooms"}>Room Count</TableColumn>
                <TableColumn key={"amount"}>Amount</TableColumn>
                <TableColumn key={"acknowledged"}>Acknowledged</TableColumn>
                <TableColumn key={"no_show"}>No Show</TableColumn>

                <TableColumn>Options</TableColumn>
              </TableHeader>
              <TableBody emptyContent={"No rows to display."} items={bookings.reverse()}>
                {(item) => (
                  <TableRow key={item.channexId}>
                    <TableCell>{item.ota_name}</TableCell>
                    <TableCell>{item.arrival_date}</TableCell>
                    <TableCell>
                      {moment(item?.arrival_date).format("MMM DD, YYYY")}{" "}
                      {" - "}
                    ...
                    <TableCell>
                      {item?.is_acknowledged ? (
                        ...
                        <Button
                          size="sm"
                          color="primary"
                          onClick={() => handleAcknowledge(item?.revision_id)}
                        >
                          Acknowledge
                        </Button>
                      )}
                    </TableCell>
                    <TableCell>{item?.no_show ? "Yes" : "No"}</TableCell>
                    <TableCell>
                      <div className="flex items-center">
                        <div
                          className="bg-blue-100 rounded-lg p-2"
                          onClick={() => {
                            handleView(item.booking_id);
                ...
                <>
                  ...
                  <ModalBody>
                    <div className="w-[100%]">
                      ...
                      <div className={`${activeTab === 1 ? "" : "hidden"}`}>
                        <div className="md:grid md:grid-cols-1 gap-[20px] bg-[#F8F8F8] rounded-[12px] p-6">
                          <div className="md:grid md:grid-cols-2 gap-[20px]">
                            ...
                            <p>
                              OTA Reservation ID :{" "}
                              {bookingDetails?.ota_reservation_code}
                            </p>
                            <p>
                              Booked At :{" "}
                              {moment(bookingDetails?.insert_at).format(
                                "MMM DD, YYYY"
                              )}
                            </p>
                            <p></p>
                            ...
                            <p>
                              Checkin Date:{" "}
                              {moment(bookingDetails?.arrival_date).format(
                                "MMM DD, YYYY"
                              )}
                              <br />
                              Checkout Date:{" "}
                              ...
                              <br />
                              Arrival Time:{" "}
                              {bookingDetails?.arrival_hour || "NA"}
                              <br />
                              Nights:{" "}
                              {moment(
                              ...
                              <br />
                              Occupancy: A:{" "}
                              {`${bookingDetails?.occupancy?.adults} `}
                      ...
                      <div
                        className={`${
                          activeTab === 4 && !bookingDetails?.no_show
                            ? ""
                            : "hidden"
                        }`}
                      >
                                ...
                                <Formik
                                  ...
                                  initialValues={{
                                    ...
                                    is_complimentary_upgrade:
                                      existingCheckIn?.is_complimentary_upgrade ||
                                      false,
                      ...
                      <div
                        className={`${
                          activeTab === 5 && !bookingDetails?.no_show
                            ? ""
                            : "hidden"
                        }`}
                      >
                                    ...
                                    <Formik
                                      initialValues={{
                                        ...
                                        complimentary_provided:
                                          existingCheckIn?.complimentary_provided,
                                        no_show:
                                          existingCheckIn?.no_show || false,
                                        is_room_changed:
                                          existingCheckIn?.is_room_changed ||
                                          false,
                                        changedRoomType:
                                          existingCheckIn?.changedRoomType ||
                                          existingCheckIn?.roomType,
                                    ...
                                    {(item) => {
                                      ...
                                      return (
                                        <TableRow key={item.id}>
                                          ...
                                          <TableCell>
                                            {item?.complimentary_provided
                                              ? "Yes"
                                              : "No"}
                                          </TableCell>
                                          <TableCell>
                                            {item.checkInDate
                                              ? moment(item.checkInDate).format(
                                                  "DD-MM-YYYY"
                                                )
                                              : "-"}
                                          </TableCell>
                      ...
                      <div
                        className={`${
                          activeTab === 6 && !bookingDetails?.no_show
                            ? ""
                            : "hidden"
                        }`}
                      >
                        <div className="">
                          <div className="flex justify-between mb-5">
                            <Select
                              ...
                              selectedKeys={[selectedCancellationPolicy]}
                            >
                              {policies.length &&
                                policies?.map((room) => (
                          ...
                          <div className="flex gap-[24px] bg-[#F8F8F8] justify-between rounded-[12px] shadow-sm mt-5 p-4">
                            <button
                              type="button"
                              className={`py-[10px] px-[70px] text-center text-white rounded-[12px] text-[25px] w-max ${"bg-[#1C1C20]"}`}
  ...
  return (
    <>
      <Dropdown backdrop="blur">
        ...
        <DropdownMenu aria-label="Link Actions">
          <DropdownItem key="Edit" onClick={handleView}>
            View
          </DropdownItem>
          {/* <DropdownItem key="ack" onClick={handleAcknowledge}>
...
export default ListPropertyBookings;
