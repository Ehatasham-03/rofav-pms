...
module.exports = {
    create: async function (req, res) {
        try {
            ...
            if (!productId || (!quantity && !noOfProducts) || !utilizationDate || !roomNumber) {
            ...
            if (utilizationQuantity > inventoryQuantity) {
                return res.status(400).json({ error: `Insufficient quantity or noOfProducts in inventory is ${inventoryQuantity} ` });
            }
            ...
            const houseKeepingManagementUtilizationEntry = await HouseKeepingManagement.create({
                uniqueId,
                propertyId,
                productId,
            ...
            console.error('Error creating houseKeepingManagementUtilizationEntry:', error);
    ...
    find: async function (req, res) {
        try {
            ...
            const houseKeepingEntries = await HouseKeepingManagement.find(query)
    ...
    findGraphDetailed: async function (req, res) {
        try {
            ...
            const houseKeepingEntries = await HouseKeepingManagement.find(query).sort('utilizationDate ASC');
            ...
            const aggregatedData = await aggregateData(houseKeepingEntries, getKeyFunction);
            ...
            sails.log.error('Error in HouseKeepingManagementController.findGraphDetailed:', error);
            return res.serverError('Internal Server Error');
        }
    },
        ...
        try {
            ...
            if (graphType) {
                switch (graphType) {
                    ...
                    case 'TopProducts':
                        result = Object.values(houseKeepingEntries.reduce((acc, entry) => {
                            if (!acc[entry.productName]) {
                                acc[entry.productName] = { productName: entry.productName, totalNoOfProducts: 0 };
                            }
                            acc[entry.productName].totalNoOfProducts += entry.noOfProducts || 0;
                ...
                result = {
                    ...
                    TopProducts: Object.values(houseKeepingEntries.reduce((acc, entry) => {
                        if (!acc[entry.productName]) {
                            acc[entry.productName] = { productName: entry.productName, totalNoOfProducts: 0 };
                        }
                        acc[entry.productName].totalNoOfProducts += entry.noOfProducts || 0;
            ...
            return res.status(200).json({
                success: true,
                data: result,
                message: `Simplified housekeeping utilization data (${graphType || 'All Graph Types'}) fetched successfully`
            });

        } catch (error) {
            sails.log.error('Error in HouseKeepingManagement.findSimplifiedGraph:', error);
            return res.serverError('Internal Server Error');
        }
    }
