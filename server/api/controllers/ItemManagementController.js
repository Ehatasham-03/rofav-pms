...
module.exports = {
    createItems: async (req, res) => {
        try {
            ...
            for (const item of items) {
                const { subCategory, mainCategory, productName, measurementUnit, weight, status } = item;

                if (!productName || !measurementUnit || !weight) {
                    return res.status(400).json({ error: 'productName, measurementUnit, and weight are required fields' });
                }
    ...
    updateItem: async (req, res) => {
        try {
            const { uniqueId } = req.params;
            const { subCategory, mainCategory, productName, measurementUnit, weight, status } = req.body;
            ...
            const updatedItem = await ItemManagement.updateOne({ uniqueId }).set(updateData);
