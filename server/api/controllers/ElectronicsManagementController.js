...
module.exports = {
    createPurchase: async function (req, res) {
        try {
            ...
            if (!productId || !vendorId) {
                return res.status(400).json({ error: 'Product Id and Vendor Id is required' });
            }
            ...
            const ElectronicsManagementPurchaseOrderEntry = await ElectronicsManagementPurchaseOrder.create({
                uniqueId,
                propertyId,
                productId,
    ...
    createUtilization: async function (req, res) {
        try {
            ...
            if (!roomNumber || (!quantity && !noOfProducts) || !dateOfInstallation || !miscellaneous) {
            ...
            const ElectronicsManagementUtilizationEntry = await ElectronicsManagementUtilization.create({
                uniqueId,
                propertyId,
                productId,
                productName: product.productName,
    ...
    findPurchases: async function (req, res) {
        try {
            ...
            const purchases = await ElectronicsManagementPurchaseOrder.find({ propertyId: propertyId });
    ...
    findUtilizations: async function (req, res) {
        try {
            ...
            const utilizations = await ElectronicsManagementUtilization.find(query)
            ...
            sails.log.error('Error in ElectronicsManagementUtilization.findUtilizations:', error);
    ...
    findGraphDetailed: async function (req, res) {
        try {
            ...
            if (miscellaneous !== undefined) query.miscellaneous = miscellaneous === 'true';
            ...
            if (dateOfInstallationStart) startDate = new Date(dateOfInstallationStart);
            if (dateOfInstallationEnd) endDate = new Date(dateOfInstallationEnd);

            query.dateOfInstallation = { '>=': startDate, '<=': endDate };

            const electronicsManagementEntries = await ElectronicsManagementUtilization.find(query)
                .sort('dateOfInstallation ASC');
            ...
            const aggregateData = async (entries, getKey) => {
                ...
                for (const entry of entries) {
                    ...
                    aggregated[key].rooms[entry.roomNumber] = (aggregated[key].rooms[entry.roomNumber] || 0) + 1;
            ...
            const aggregatedData = await aggregateData(electronicsManagementEntries, getKeyFunction);
            ...
            sails.log.error('Error in ElectronicsManagementController.findGraphDetailed:', error);
            return res.serverError('Internal Server Error');
        }
    },
        ...
        try {
            ...
            if (graphType) {
                switch (graphType) {
                    case 'ByOverTime':
                        result = electronicsManagementEntries.reduce((acc, entry) => {
                            ...
                            if (!acc[dateKey]) {
                                acc[dateKey] = { dateOfInstallation: dateKey, totalNoOfProducts: 0 };
                            }
                    ...
                    case 'TopProducts':
                        result = Object.values(electronicsManagementEntries.reduce((acc, entry) => {
                            if (!acc[entry.productName]) {
                                acc[entry.productName] = { productName: entry.productName, totalNoOfProducts: 0 };
                            }
                            acc[entry.productName].totalNoOfProducts += entry.noOfProducts || 0;
                ...
                result = {
                    ...
                    TopProducts: Object.values(electronicsManagementEntries.reduce((acc, entry) => {
                        if (!acc[entry.productName]) {
                            acc[entry.productName] = { productName: entry.productName, totalNoOfProducts: 0 };
                        }
                        acc[entry.productName].totalNoOfProducts += entry.noOfProducts || 0;
            ...
            return res.status(200).json({
                success: true,
                data: result,
                message: `Simplified electronics utilization data (${graphType || 'All Graph Types'}) fetched successfully`
            });

        } catch (error) {
            sails.log.error('Error in ElectronicsManagement.findSimplifiedGraph:', error);
            return res.serverError('Internal Server Error');
        }
    }
