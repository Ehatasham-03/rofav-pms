...
module.exports = {
    find: async function (req, res) {
        try {
            const { uniqueId, propertyId, purchaseOrderId, productId, productName, mainCategoryId, mainCategoryName, subCategoryId, subCategoryName, damageFrom, refundStatus, purchasedDamageItemStatus, inHouseDamageItemStatus, vendorUniqueId } = req.query;
    ...
    update: async function (req, res) {
        try {
            ...
            if (updateData.purchasedDamageItemStatus) {
                updatedFields.purchasedDamageItemStatus = updateData.purchasedDamageItemStatus;
            }

            if (updateData.inHouseDamageItemStatus) {
                updatedFields.inHouseDamageItemStatus = updateData.inHouseDamageItemStatus;
            }

            const updatedItem = await MMDamagedItems.updateOne({ uniqueId })
                .set(updatedFields)
                .fetch();
            ...
            return res.json({ message: 'Damaged item updated successfully', data: updatedItem });
        } catch (error) {
            console.error(error);
            return res.status(500).json({ error: 'Internal Server Error' });
        }
