...
module.exports = {
    createMainCategory: async (req, res) => {
        try {
            ...
            const nameAlreadyExists = await MainCategory.findOne({ name });
            if (nameAlreadyExists) {
                return res.status(400).json({ error: 'Name already exists' });
            }

            const uniqueId = uuidv4();
            const mainCategory = await MainCategory.create({ uniqueId, name, status: status || 'false' }).fetch();
    ...
    createSubCategory: async (req, res) => {
        try {
            ...
            if (mainCategoryId) {
                mainCategory = await MainCategory.findOne({ uniqueId: mainCategoryId });
            }
            ...
            const uniqueId = uuidv4();
            const subCategory = await SubCategory.create({ uniqueId, name, mainCategory: mainCategory ? mainCategory.uniqueId : null, status: status || 'false' }).fetch();
    ...
    getMainCategories: async (req, res) => {
        try {
            const { includeSubCategories, uniqueId } = req.query;
            let query = {};
            ...
            if (includeSubCategories === 'true') {
                mainCategories = await MainCategory.find(query).populate('subCategories');
            } else {
    ...
    getSubCategories: async (req, res) => {
        try {
            const { mainCategoryId, includeMainCategory, uniqueId } = req.query;
            let query = {};
            ...
            if (includeMainCategory === 'true') {
                subCategories = await Promise.all(
                    subCategories.map(async (subCategory) => {
                        const mainCategory = await MainCategory.findOne({ uniqueId: subCategory.mainCategory });
    ...
    updateMainCategory: async (req, res) => {
        try {
            ...
            const updatedMainCategory = await MainCategory.updateOne({ uniqueId }).set(updateData);
    ...
    updateSubCategory: async (req, res) => {
        try {
            ...
            const updatedSubCategory = await SubCategory.updateOne({ uniqueId }).set(updateData);
