...
module.exports = {
    createVendor: async (req, res) => {
        try {
            ...
            if (!vendorCategories || vendorCategories.length === 0) {
                return res.status(400).json({ error: 'Vendor Categories are required' });
            }

            const mainCategories = await MainCategory.find({ uniqueId: vendorCategories });
            ...
            if (selfVending === 'true') {
                vendorData.vendorName = `Self Vendor ${mainCategories.map((category) => category.name).join(', ')}`;
                vendorData.vendorPhoneNumber = '1234567890';
            } else {
                if (!vendorName || !vendorPhoneNumber) {
                    return res.status(400).json({
                        error: 'Vendor Name and Phone Number are required when selfVending is false',
                    });
                }
    ...
    updateVendor: async (req, res) => {
        try {
            ...
            const updatedVendor = await VendorsManagement.updateOne({ uniqueId: uniqueId }).set(updateData);
    ...
    getVendors: async (req, res) => {
        try {
            const { uniqueId, vendorCategory, selfVending, vendorStatus } = req.query;
            let query = {};
            ...
            if (vendorCategory) {
                const mainCategory = await MainCategory.findOne({ uniqueId: vendorCategory });
                if (!mainCategory) {
            ...
            const vendors = await VendorsManagement.find(query).populate('vendorCategories');

            const vendorsWithCategories = vendors.map(async (vendor) => {
                const categories = await Promise.all(
                    vendor.vendorCategories.map(async (category) => {
                        const mainCategory = await MainCategory.findOne({ uniqueId: category.mainCategory });
                        return mainCategory;
                    })
                );
