...
module.exports = {
    createMarketItems: async (req, res) => {
        try {
            ...
            if (!Array.isArray(marketItems) || marketItems.length === 0) {
                return res.status(400).json({ error: 'Market items array is required and must not be empty' });
            }
            ...
            for (const marketItem of marketItems) {
                ...
                if (!productName) {
                    return res.status(400).json({ error: 'productName is a required field' });
                }

                if (!subCategory || !mainCategory) {
                    return res.status(400).json({ error: 'subCategory and mainCategory are required fields' });
                }

                const subCategoryExists = await SubCategory.findOne({ where: { uniqueId: subCategory } }).populate('mainCategory');
                if (!subCategoryExists) {
                    return res.status(404).json({ error: 'SubCategory not found' });
                }

                const mainCategoryExists = await MainCategory.findOne({ where: { uniqueId: mainCategory } });
                ...
                if (subCategoryExists.mainCategory.uniqueId !== mainCategory) {
                    return res.status(400).json({ error: 'The specified sub category does not belong to the main category' });
                }
    ...
    deleteMarketItem: async (req, res) => {
        try {
            ...
            return res.status(200).json({ message: 'Market item deleted successfully' });
        } catch (error) {
            console.error(error);
            return res.status(500).json({ error: 'Internal server error' });
        }
        ...
        try {
            ...
            const marketItemsWithDetails = await Promise.all(
                marketItems.map(async (marketItem) => {
                    const mainCategoryDetails = await MainCategory.findOne({ where: { uniqueId: marketItem.mainCategory } });
                    const subCategoryDetails = await SubCategory.findOne({ where: { uniqueId: marketItem.subCategory } });

                    marketItem.mainCategoryName = mainCategoryDetails ? mainCategoryDetails.name : null;
                    marketItem.subCategoryName = subCategoryDetails ? subCategoryDetails.name : null;
    ...
    updateMarketItem: async (req, res) => {
        try {
            ...
            const updatedMarketItem = await MarketManagement.updateOne({ uniqueId }).set(updateData);

            if (!updatedMarketItem) {
                return res.status(404).json({ error: 'Market item not found' });
            }

            return res.status(200).json({ message: 'Market item updated successfully' });
        } catch (error) {
            console.error(error);
            return res.status(500).json({ error: 'Internal server error' });
        }
