...
module.exports = {
    create: async function (req, res) {
        try {
            ...
            for (const item of items) {
                const { productId, quantity, unit, noOfProducts, vendorId, incomingDate, expiryDate } = item;
                if (!productId || !quantity || !unit || !noOfProducts || !vendorId || !incomingDate || !expiryDate) {
                    responses.push({
                        status: 400,
                        message: 'Product ID, quantity, unit, number of products, vendor ID, incoming date, and expiry date are required for each item'
                    });
                    continue;
                }
                ...
                if (existingEntry) {
                    ...
                    const updatedEntry = await InventoryManagement.updateOne({ uniqueId: existingEntry.uniqueId })
                        .set({
                            quantity: existingEntry.quantity + quantity,
                            noOfProducts: existingEntry.noOfProducts + noOfProducts,
                            incomingDate,
                            expiryDate,
                            status: vendor.selfVending === 'true' ? 'InHouse' : existingEntry.status
                        })
                        .fetch();
                    responses.push({
                        status: 200,
    ...
    find: async function (req, res) {
        try {
            const { propertyId, vendorId, expiryDate, incomingDate, productId } = req.query;
    ...
    update: async function (req, res) {
        try {
            ...
            if (!uniqueId) {
                return res.status(400).json({ error: 'Inventory entry uniqueId is required' });
            }

            const existingEntry = await InventoryManagement.findOne({ uniqueId });

            if (!existingEntry) {
                return res.status(404).json({ error: `Inventory entry with uniqueId ${uniqueId} not found` });
            }
            ...
            const updatedEntry = await InventoryManagement.updateOne({ uniqueId })
                .set(updatedFields)
                .fetch();

            return res.json({ message: 'Inventory entry updated successfully', data: updatedEntry });
        } catch (error) {
            console.error(error);
            return res.status(500).json({ error: 'Internal Server Error' });
        }
    ...
    delete: async function (req, res) {
        try {
            ...
            const deletedEntry = await InventoryManagement.destroyOne({ uniqueId: uniqueId });
