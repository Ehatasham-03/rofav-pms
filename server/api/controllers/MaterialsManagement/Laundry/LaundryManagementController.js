...
module.exports = {
  createOut: async function (req, res) {
    try {
      const { propertyId, items, outDate } = req.body;
      ...
      for (const item of items) {
        ...
        if (!inventoryEntry) {
          return res.status(404).json({
            error: `In House entry not found for product ${productName} and property ${propertyId}`,
          });
        }
  ...
  createIn: async function (req, res) {
    try {
      const { uniqueId, propertyId, inDate, isDamaged, damagedItems } =
        req.body;
      ...
      if (isDamaged && damagedItems) {
        ...
        if (!inventoryEntry) {
          return res.status(404).json({
            error: `In House entry not found for product ${laundryManagementEntry.productId} and property ${propertyId}`,
          });
        }
  ...
  downloadExcel: async function (req, res) {
    try {
      const { propertyId, vendorId, status } = req.query;
